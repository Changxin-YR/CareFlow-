...
DOCUMENTS = [
    ...
    _d("CORE002", "基层慢性病健康管理服务能力建设指引", "国家卫生健康委",
       ...
       notes="page body is authoritative; attachment PDF optional supplement; 官方附件 PDF 为纯扫描件（PyMuPDF 抽取 0 字符 / 10 页），需 OCR 才能全文入库"),
    ...
    _d("HTN002", "国家基层高血压防治管理指南 2025版", "国家心血管病中心",
       "P3", "professional_guideline", "2025版", "2025-09-24",
       ["HYPERTENSION", "CARDIOVASCULAR"],
    ...
    _d("DM002", "中国2型糖尿病防治指南（2020年版）", "中华医学会糖尿病学分会",
       "P3", "professional_guideline", "2020版", "2021-04-27",
       ["DIABETES"],
       ["followup", "risk_assessment", "medication_safety", "lifestyle", "referral"],
       "KB_DM", "03_diabetes",
       "https://rs.yiigle.com/CN2021/1315505.htm",
       notes="pending manual download: publisher paywall (Yiigle), no official free full text; 正式出处 中华糖尿病杂志 2021;13(4):315-409, DOI 10.3760/cma.j.cn115791-20210221-00095"),
    _d("DM003", "国家基层糖尿病防治管理指南（2022）", "国家基层糖尿病防治管理办公室",
       "P3", "professional_guideline", "2022", "2022-03-01",
       ...
       notes="pending manual download: third-party portal requires login/authorization; 正式出处 中华内科杂志 2022;61(3):249-262, DOI 10.3760/cma.j.cn112138-20220120-000063"),
    # ---- 04_copd -----------------------------------------------------------
    ...
    _d("MULTI001", "“三高”共管规范化诊疗中国专家共识（2023版）", "中华医学会等",
       "P3", "expert_consensus", "2023版", "2023-07-24",
    ...
    # ---- 06_lifestyle ------------------------------------------------------
    _d("LIFE001", "高血压营养和运动指导原则（2024年版）", "国家卫生健康委",
       "P1", "nhc_policy", "2024版", "2024-07-01",
       ["HYPERTENSION", "LIFESTYLE", "MULTIMORBIDITY"],
       ...
       kind="pdf",
       notes="官方统一入口页含 4 份附件；本条目对应同名《高血压营养和运动指导原则（2024年版）》；附件为扫描件（文本层仅约 388 字符），需 OCR 才能全文入库"),
    _d("LIFE001A", "高血糖症营养和运动指导原则（2024年版）", "国家卫生健康委",
       "P1", "nhc_policy", "2024版", "2024-07-01",
       ["DIABETES", "LIFESTYLE"],
       ["lifestyle", "education", "followup"],
       "KB_LIFESTYLE", "06_lifestyle",
       "https://www.nhc.gov.cn/ylyjs/gzdt/202407/256b4eb8398440a8811344c7be50a333.shtml",
       kind="pdf",
       notes="官方统一入口页第 2 份附件；扫描件（文本层仅约 391 字符），需 OCR 才能全文入库"),
    _d("LIFE001B", "高脂血症营养和运动指导原则（2024年版）", "国家卫生健康委",
       "P1", "nhc_policy", "2024版", "2024-07-01",
       ...
       "https://www.nhc.gov.cn/ylyjs/gzdt/202407/256b4eb8398440a8811344c7be50a333.shtml",
       kind="pdf",
       notes="官方统一入口页第 3 份附件；扫描件（文本层仅约 405 字符），需 OCR 才能全文入库"),
    _d("LIFE001C", "高尿酸血症营养和运动指导原则（2024年版）", "国家卫生健康委",
       "P1", "nhc_policy", "2024版", "2024-07-01",
       ...
       "https://www.nhc.gov.cn/ylyjs/gzdt/202407/256b4eb8398440a8811344c7be50a333.shtml",
       kind="pdf",
       notes="官方统一入口页第 4 份附件；扫描件（文本层仅约 418 字符），需 OCR 才能全文入库"),
    ...
    _d("LIFE007", "成人慢性肾脏病食养指南（2024年版）", "国家卫生健康委",
       ...
       "https://www.nhc.gov.cn/sps/c100088/202402/9ba512ba8e314a47a181db11d2fa188d.shtml",
       notes="shared official entry page for four 食养指南 (2024); annex PDF on the same page"),
    _d("LIFE008", "居民体重管理核心知识（2024年版）", "国家卫生健康委",
    ...
    _d("LIFE008A", "居民体重管理核心知识（2024年版）释义", "国家卫生健康委",
       ...
       "https://www.nhc.gov.cn/ylyjs/gzdt/202407/9ec6136773bc41048a39f275fcc37b44.shtml",
       kind="pdf",
       notes="官方附件『释义』；文本层可用（4 页约 1974 字符）"),
    # ---- 07_primarycare ----------------------------------------------------
    ...
    _d("PRIM003", "老年人健康管理技术规范 WS/T 484—2015", "国家卫生计生委",
       ...
       notes="标准发布页；官方 PDF（39 页）的 ToUnicode CMap 损坏：中文被映射到 U+00xx 控制区、数字映射为全角、字母映射到 U+72xx，抽出如 犐犆犛１１．０２０（应 ICS 11.020），无法算术还原；已改用发布页元数据占位，需 OCR 或替换带正确 ToUnicode 的同版 PDF"),
    # ---- 08_who ------------------------------------------------------------
    _d("WHO001", "WHO PEN (Package of essential NCD interventions)", "World Health Organization",
       "P4", "who_guideline", "2nd edition", "2020-09-07",
...
EXTRA_URLS = {
    # 旧 iris.who.int/bitstream/handle/... 已失效（返回 755 字节 HTML），改用新版 API 直链
    "WHO001": [
        "https://iris.who.int/server/api/core/bitstreams/b9f09202-a320-4c07-ba2c-afe0d1186339/content"
    ],
    "WHO002": [
        "https://iris.who.int/server/api/core/bitstreams/aef55845-a638-45d0-b816-70dd5bf8885a/content"
    ],
    "WHO003": [
        "https://iris.who.int/server/api/core/bitstreams/0c21ba41-60db-4592-a97c-60a8b62033eb/content"
    ],
}
